...
if (isset($_POST['login_usuario'])) {
    ...
    if ($stmt) {
        ...
        if ($resultado && mysqli_num_rows($resultado) > 0) {
            ...
            $_SESSION['mensagem'] = 'Email não existe no banco de dados.';
            header('Location: login.php'); exit;
        }
    } else {
        $_SESSION['mensagem'] = 'Erro servidor. Banco Caiu? Tente depois.';
        header('Location: login.php'); exit;
    }
}
?>
...
<head>
    ...
    <style>
        ...
        .login-box { border: none; border-radius: 1rem; box-shadow: 0 10px 25px rgba(0,0,0,0.1); width: 100%; max-width: 450px; background: white;}
        .card-header-icon { background: #16191c; width: 70px; height: 70px; border-radius: 50%; display: flex; align-items: center; justify-content: center; position: absolute; top: -35px; left: 50%; transform: translateX(-50%); box-shadow: 0 5px 15px rgba(0,0,0,0.2); }
    </style>
</head>
<body>
    ...
    <div class="login-box p-4 mt-4 position-relative">
        ...
        <h3 class="text-center mt-5 mb-4 fw-bold text-dark">Portal Seguros</h3>
        ...
        <form method="POST">
            ...
            <div class="form-floating mb-3">
               ...
               <label for="usuario" class="text-secondary"><i class="bi bi-envelope me-2"></i>Email Cadastrado</label>
            </div>
            ...
            <button type="submit" name="login_usuario" class="btn btn-warning w-100 py-3 mb-4 rounded-3 shadow fw-bold text-dark fs-5">AUTENTICAR <i class="bi bi-arrow-right ms-2"></i></button>
        </form>
        
        <hr class="text-muted border-dashed border-2">

        <p class="text-center mb-1 text-secondary mt-3">É sua primeira vez aqui?</p>
        <a href="register.php" class="btn btn-light w-100 rounded-pill border fw-semibold">Quero criar minha nova Conta</a>

        <div class="text-center mt-3">
             <a href="../../public/index.php" class="text-decoration-none text-muted small"><i class="bi bi-arrow-left"></i> Retornar pro inìcio da FSA Site</a>
        </div>
    </div>
